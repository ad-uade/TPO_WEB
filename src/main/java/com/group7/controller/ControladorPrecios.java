...
import com.group7.service.AdministradorPrecios;
import com.group7.service.AdministradorProveedor;
...
public class ControladorPrecios extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("delete") != null) {
			...
			ProveedorVO p = AdministradorProveedor.getInstancia()
					.getProveedor(Integer.valueOf(request.getParameter("proveedorid")));
		...
		if ("default".equals(action)) {
			...
			rodamientos = AdministradorCotizacion.getInstancia().getRodamientos(); // Chequear
																					// que
																					// sea
																					// correcto,
																					// ya
			...
			if (items != null && precios != null) {
				if (items.size() > 0 && precios.size() > 0) {
					...
					AdministradorPrecios.getInstancia().armarListaDePrecios(p, items, precios, tipo, vigencia,
							descuento);
					cerrar();
				}
			}
			...
			if (items == null && precios == null) {
				...
				for (int i = 0; i < items.size(); i++) {
					...
					if (ro.getCodigoPieza().equals(unR.getCodigoPieza())
						...
						precios.set(i, precioVenta);// No sumo el precio
													// anterior, lo reemplazo.
						existe = true;
					}
				}
				if (!existe) {
